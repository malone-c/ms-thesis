...
I now propose a model for the case discussed above. This discussion builds on chapter 3, where I outlined the model in broad generality. There, I discussed the model while leaving some key situation-specific hyperparameters and distributions unspecified. For example, the prior distribution on the initial population size $N_1$, and the relationship between probability of capture given distance between fly and trap $p(c \mid d)$ were unspecified.^[In that chapter, distance was denoted as $d_{i,t,k}$, to stress that distance was measured between a given fly $i$ and a given trap $k$ in a given week $t$. Here, I denote distance $d$, under the assumption that distance is still understood to be between a fly and a trap.] Here, these gaps are filled. Hyperparameters and distributions are proposed and justified.
...
### Prior distribution on fly population size {#popn-size}
...
### Prior distribution on fly spatial locations
