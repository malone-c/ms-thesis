...
### Reasons to distrust this model

1. There may be many cases where flies are not eradicated, but the population size is unsustainably small.
2. No uncertainty about the probability of capture.
3. Growth rates are not weather dependent.
4. Dispersal data is used twice.
5. The location prior and likelihood use data from different sources.
6. Our model predicts that medfly is unlikely to survive uncontrolled in Adelaide. This contradicts a status quo view (@PIRSA, the chapter on eradication).

### Model validation

#### Model validation is hard for this case
